...
# img fondo 2
imagenfondo2 = "background.png"
imagenf2 = pygame.image.load('background.jpg')
imagenf2 = pygame.transform.scale(imagenf2, (1540, 880))
...
def show_lila_window():
    lila_window = Window("Ventana Lila", lilacolor)
    ...
    while running:
        ...
        lila_window.show()
        if not button_grey.rect.collidepoint(pygame.mouse.get_pos()):
            lila_window.window.blit(imagenf1, (width // 2 - imagenf1.get_width() // 2, 30))
        button_grey.draw(lila_window.window)
        pygame.display.flip()
    pygame.quit()


def show_lightblue_window():
    celeste_window = Window("Ventana Celeste", lightbluecolor)
    button_grey = Button(25, 25, 50, 50, colorhumilde, show_grey_window)
    running = True
    while running:
        for event in pygame.event.get():
        ...
        celeste_window.show()
        if not button_grey.rect.collidepoint(pygame.mouse.get_pos()):
            celeste_window.window.blit(imagenf1, (width // 2 - imagenf1.get_width() // 2, 30))
...
button_celeste = Button(width//2 - 700, height//2 + 150, 500, 100, (105, 155, 210), show_lightblue_window)
